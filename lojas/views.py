...
def detalhe_loja(request, loja_id):
    ...
    if request.method == 'POST' and 'finalizar_compra' in request.POST:
        cartao = request.POST.get('num_cartao')
        print(f"PAGAMENTO REALIZADO COM SUCESSO NO CARTÃO: {cartao}")

        request.session['carrinho'] = {}
        return redirect(request.path + '?sucesso=true')
    ...
    return render(request, 'lojas/detalhe.html', {
        'loja': loja, 
        'produtos': produtos,
        'itens_carrinho': itens_carrinho,
        'total_carrinho': total_carrinho,
